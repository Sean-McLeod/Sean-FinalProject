...
class ButtonClass:
    ...
    def draw_button(self, screen, text_size, outline=None):
        # draw button
        if outline:
            pygame.draw.rect(screen, outline, (self.position_x - 2, self.position_y - 2, self.width + 4, self.height + 4), 0)

        pygame.draw.rect(screen, self.color, (self.position_x, self.position_y, self.width, self.height), 0)
        ...
        if self.text != '':
            ...
            screen.blit(text, (
            self.position_x + (self.width / 2 - text.get_width() / 2), self.position_y + (self.height / 2 - text.get_height() / 2)))
