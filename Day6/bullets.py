...
class BulletClass(pygame.sprite.Sprite):
    def __init__(self, pos_x, pos_y, x_speed, y_speed):
        super().__init__()
        self.image = pygame.transform.rotate(pygame.image.load(constants.BULLET_IMAGE), constants.ROTATE_THREE_TIMES)
        self._pos_y = pos_y
        self._pos_x = pos_x
        self._x_speed = x_speed
        self._y_speed = y_speed
        self.rect = pygame.Rect(self._pos_x, self._pos_y, self.image.get_width(), self.image.get_height())
